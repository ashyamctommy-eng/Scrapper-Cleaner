...
❯ /scr @nativecodes 1000
❯ /scr @nativecodes 1000 515462
❯ /scr @nativecodes 1000 BankName
❯ /scr t.me/nativecodes 1000
❯ /scr https://t.me/+ZBqGFP5RpmY2Y1 1000
...
async def scrape_messages(client, channel_username, limit, start_number=None, bank_name=None):
    ...
    async for message in user.search_messages(channel_username):
        ...
        if text:
            if bank_name and bank_name.lower() not in text.lower():
                continue
            matched_messages = re.findall(pattern, text) # @nativecodes
            if matched_messages:
                formatted_messages = []
    ...
    return messages[:limit] # @nativecodes
    ...
    if unique_messages:
        try:
            ...
            async with aiofiles.open(file_name, mode='w') as f:
                await f.write("\n".join(unique_messages)) # @nativecodes)

            user_link = await get_user_link(message)
            caption = (
                f"『💳 𝐂𝐑𝐄𝐃𝐈𝐓 𝐂𝐀𝐑𝐃 𝐒𝐂𝐑𝐀𝐏𝐄𝐑 💳』\n\n"
            ...
            try:
                if os.path.exists(file_name):
                    os.remove(file_name) # @nativecodes
            except Exception as e:
        ...
        try:
            ...
            logger.error(f"Error sending no results message: {e}") # @nativecodes
...
async def send_join_request(client, invite_link, message):
    try:
        ...
        logger.info(f"Join request sent to the chat: {invite_link}")
        await message.edit_text("<b>𝐇𝐞𝐲 𝐁𝐫𝐨 𝐈 𝐇𝐚𝐯𝐞 𝐒𝐞𝐧𝐭 𝐉𝐨𝐢𝐧 𝐑𝐞𝐪𝐮𝐞𝐬𝐭✅</b>") # @nativecodes
        return False
...
def setup_scr_handler(app):
    ...
    async def scr_cmd(client, message):
        ...
        if channel_identifier.lstrip("-").isdigit():
            ...
            if channel_identifier.startswith("https://t.me/+"):
                ...
                channel_username = channel_identifier # @nativecodes
        ...
        unique_messages, duplicates_removed = remove_duplicates(scrapped_results) # @nativecodes
    ...
    async def mc_cmd(client, message):
        ...
        temporary_msg = await client.send_message(message.chat.id, "<b>𝐒𝐜𝐫𝐚𝐩𝐢𝐧𝐠 𝐈𝐧 𝐏𝐫𝐨𝐠𝐫𝐞𝐬𝐬</b>", reply_to_message_id=message.id) # @nativecodes
        ...
        for result in results:
            all_messages.extend(result) # @nativecodes
...
async def scrape_messages_task(client, channel_username, limit, bot_client, message):
    try:
        ...
        if channel_username.startswith("https://t.me/+"):
            ...
            chat = await client.get_chat(channel_username) # @nativecodes
...
async def clean_cmd(client, message):
    ...
    status_msg = await message.reply_text("<b>𝐏𝐫𝐨𝐜𝐞𝐬𝐬𝐢𝐧𝐠 𝐟𝐢𝐥𝐞...</b>") # @nativecodes
    ...
    try:
        ...
        for line in lines:
            if line.strip():
                ...
                if normalized:
                    cleaned_cards.append(normalized) # @nativecodes
        ...
        if unique_cards:
            ...
            os.remove(file_path) # @nativecodes
